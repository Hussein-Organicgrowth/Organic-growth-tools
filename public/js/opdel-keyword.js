...
submit.addEventListener("click", async function (event) {
  ...
  await generate(keywordsValue);
});

async function generate(keywordsString) {
  const CHUNK_SIZE = 200;
  let keywords = keywordsString.split("\n");

  // Split the keywords into chunks of 200
  const chunks = [];
...
async function processChunk(chunk) {
  ...
  prompt = prompt.replace("{søgeord}", chunk.join(", "));

  // rest of your prompt setup...
  let titlePrompt =
...
async function processChunkeee(keywords) {
  // Create a new AbortController instance
  textArea.value = "";
  controller = new AbortController();
  const signal = controller.signal;
